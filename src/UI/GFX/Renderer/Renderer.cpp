...
inline std::vector<SpriteInstance> record_draw_list(const Canvas *canvas)
{
  ...
  query.each([&instanceList, &counter](ecs::Entity e, 
    ...
    instanceList[counter] = {
      .position = { 
        static_cast<float>(baseComponent.rect.x), 
  ...
  UI_LOG_MSG("Reporting %i entities", instanceList.size());
...
void ui::draw(const Window *window)
{
  ...
  if (!SDL_WaitAndAcquireGPUSwapchainTexture(cmd, windowPtr, &swapchainTexture, nullptr, nullptr)) {
  ...
  if (swapchainTexture != nullptr) {
    ...
    const SDL_GPUTransferBufferLocation transferBufferLocation = {
      .transfer_buffer = drawPipeline.spriteDataTransferBuffer,
      .offset = 0
    };
